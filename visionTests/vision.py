...
def main():
	...
	with dai.Pipeline() as pipeline:
		...
		dt = 1.0 / ROBOT_FREQUENCY  # control/update period (seconds)
		max_step_xy = 0.002  # max change in x/y per cycle (meters)
		...
		while pipeline.isRunning():
			...
			if red_center is not None:
				...
				if world is not None:
					...
					# smooth update: move a small step towards target each cycle
					dx = x_w - target_pose[0]
					dy = y_w - target_pose[1]
					dz = z_w - target_pose[2]

					# clamp step size for smooth motion
					step_x = np.clip(dx, -max_step_xy, max_step_xy)
					step_y = np.clip(dy, -max_step_xy, max_step_xy)
					step_z = np.clip(dz, -max_step_xy, max_step_xy)

					target_pose[0] += step_x
					target_pose[1] += step_y
					target_pose[2] += step_z

					# use servoL for continuous Cartesian following
					rtde_c.servoL(
						target_pose,
						0.5,  # speed
						0.5,   # acceleration
			...
			cv2.imshow("Vision object tracking - continuous", frame)
